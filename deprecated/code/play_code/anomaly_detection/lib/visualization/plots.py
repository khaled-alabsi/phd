...
for anomaly detection analysis and model comparison.
"""
...
def plot_mcusum_diagnostics(
    ...
    Args:
        ...
        statistics_anomaly: MCUSUM statistics for anomaly data (optional)
        h: Control limit threshold
        title_suffix: Suffix to add to plot titles
        use_log: Whether to use log scale for y-axis
        diff_threshold: Threshold for marking significant differences
    """
...
def plot_time_series_feature(X_incontrol: np.ndarray, 
                           X_outcontrol: np.ndarray,
                           feature_idx: int = 10,
                           figsize: Tuple[int, int] = (15, 6)) -> None:
    """
    Plot time series for a specific feature comparing in-control vs out-of-control data.
    
    Args:
        X_incontrol: In-control data
        X_outcontrol: Out-of-control data
        feature_idx: Index of feature to plot
        figsize: Figure size
    """
    plt.figure(figsize=figsize)
    plt.plot(X_incontrol[:, feature_idx], label='In-Control Data', color='blue')
    plt.plot(X_outcontrol[:, feature_idx], label='Out-of-Control Data', color='red')
    plt.title(f'Time Series Plot of Scaled Data (Feature {feature_idx})')
    plt.xlabel('Time')
...
def analyze_results(df_results: pd.DataFrame, summary_df: pd.DataFrame):
    ...
    Returns:
        ModelComparisonAnalyzer instance for further analysis
    """
    # Initialize analyzer
